...
class MeShowsRequest(object):

    """Implementation of the 'Me Shows Request' model.

    Attributes:
        ids (List[str]): A JSON array of the [Spotify
            IDs](https://developer.spotify.com/documentation/web-api/#spotify-u
            ris-and-ids).   A maximum of 50 items can be specified in one
            request. *Note: if the `ids` parameter is present in the query
            string, any IDs listed here in the body will be ignored.*
    ...
    def __init__(self,
        ...
        """Constructor for the MeShowsRequest class"""
